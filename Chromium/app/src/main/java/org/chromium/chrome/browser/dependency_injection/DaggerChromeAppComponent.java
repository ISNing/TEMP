...
public final class DaggerChromeAppComponent implements ChromeAppComponent {
          ...
          if (local instanceof MemoizedSentinel) {
            local =
                new CloseButtonNavigator(
                    getBrowserServicesActivityTabController(), resolveTabProvider());
    ...
    private TrustedWebActivityDisclosureController getTrustedWebActivityDisclosureController() {
      return TrustedWebActivityDisclosureController_Factory
          .newTrustedWebActivityDisclosureController(
              ChromeAppModule_ProvidesChromePreferenceManagerFactory
                  .proxyProvidesChromePreferenceManager(
                      DaggerChromeAppComponent.this.chromeAppModule),
    ...
    private final class SwitchingProvider<T> implements Provider<T> {
      ...
      public T get() {
        switch (id) {
          ...
          case 2: // org.chromium.chrome.browser.compositor.CompositorViewHolder
            return (T)
          ...
          case 3: // org.chromium.chrome.browser.browserservices.trustedwebactivityui.controller.TwaVerifier
            return (T) getTwaVerifier();

          case 4: // org.chromium.chrome.browser.snackbar.SnackbarManager
            return (T)
                ChromeActivityCommonsModule_ProvideSnackbarManagerFactory
                    .proxyProvideSnackbarManager(chromeActivityCommonsModule);

          case 5: // org.chromium.chrome.browser.browserservices.trustedwebactivityui.splashscreen.TwaSplashController
            return (T) getTwaSplashController();

          case 6: // org.chromium.chrome.browser.customtabs.features.ImmersiveModeController
            return (T) resolveImmersiveModeController();

          case 7: // org.chromium.chrome.browser.browserservices.trustedwebactivityui.controller.ClientAppDataRecorder
            return (T) getClientAppDataRecorder();

          case 8: // org.chromium.chrome.browser.customtabs.CustomTabTopBarDelegate
            return (T) getCustomTabTopBarDelegate();

          case 9: // org.chromium.chrome.browser.customtabs.CustomTabBottomBarDelegate
            return (T) resolveBottomBarDelegate();

          case 10: // org.chromium.chrome.browser.customtabs.dynamicmodule.DynamicModuleToolbarController
            return (T) resolveDynamicModuleToolbarController();

          case 11: // org.chromium.chrome.browser.customtabs.CustomTabDelegateFactory
            return (T) getCustomTabDelegateFactory();

          case 12: // org.chromium.ui.base.ActivityWindowAndroid
            return (T)
                ChromeActivityCommonsModule_ProvideActivityWindowAndroidFactory
                    .proxyProvideActivityWindowAndroid(chromeActivityCommonsModule);
  ...
  private final class WebappActivityComponentImpl implements WebappActivityComponent {
    ...
    private volatile Object webappDelegateFactory = new MemoizedSentinel();
    ...
    @Override
    public WebappDelegateFactory resolveWebappDelegateFactory() {
      Object local = webappDelegateFactory;
      if (local instanceof MemoizedSentinel) {
        synchronized (local) {
          local = webappDelegateFactory;
          if (local instanceof MemoizedSentinel) {
            local =
                new WebappDelegateFactory(
                    ChromeActivityCommonsModule_ProvideChromeActivityFactory
                        .proxyProvideChromeActivity(chromeActivityCommonsModule),
                    getCustomTabBrowserControlsVisibilityDelegate());
            webappDelegateFactory = DoubleCheck.reentrantCheck(webappDelegateFactory, local);
          }
        }
      }
      return (WebappDelegateFactory) local;
    }
